...
def box_cxcywh_to_xyxy(x: Tensor) -> Tensor:
    """Performs box-coordinates conversion.
...
def box_xyxy_to_cxcywh(x: Tensor) -> Tensor:
    """Performs box-coordinates conversion, from Pascal VOC.

    Args:
        x: boxes are expected to be in (x1, y1, x2, y2) format.

    Returns:
        Boxes in (x_c, y_c, w, h) format.
...
def box_iou(boxes1: Tensor, boxes2: Tensor) -> Tuple[Tensor]:
    """Returns intersection-over-union (Jaccard index) of boxes.

    Both sets of boxes are expected to be in (x1, y1, x2, y2) format.

    Args:
        boxes1: bounding boxes.
        boxes2: bounding boxes.

    Returns:
        iou: the NxM matrix containing the pairwise IoU values for every element in boxes1 and boxes2.
